...
Ottimizzato per nodo CPU EPYC (32 core).
"""
...
REPO_TYPE  = "dataset"
OUT_DIR    = Path("50d/full_dataset")
SPLITS     = ["train", "val", "test"]

# https://share.gemini.google/fDXK2GJcTi7t decisione parametri
UMAP_PARAMS = dict(
    n_neighbors  = 30,         # Aumentato per catturare meglio la struttura globale in 50D
    min_dist     = 0.0,        # Ridotto a 0.0 se devi fare clustering (altrimenti lascia 0.1)
    n_components = 50,
    metric       = 'cosine',   # <--- IMPORTANTE per gli embedding di testo!
    random_state = 42,
    verbose      = True,
    low_memory   = False, 
    n_jobs       = -1     
...
log.info("Output shape: %s", emb_50d.shape)
# ── 4. Salvataggio ─────────────────────────────────────────────────────────────
