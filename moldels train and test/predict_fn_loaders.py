...
# ─────────────────────────────────────────────────────────────────────────
# Loader "grezzi" (model, tokenizer, device) — usati da xai_utils.py per
# saliency/integrated gradients/attention, che a differenza di predict_fn
# hanno bisogno di accedere ai gradienti o alle attention weights, non solo
# alle probabilità in output.
# ─────────────────────────────────────────────────────────────────────────
def load_model_fcnn(checkpoint_dir=None, device=None):
...
def load_model_papercnn(checkpoint_dir=None, device=None):
    from data_utils import get_tokenizer, MAX_LENGTH
    from train_papercnn import PaperCNN, CHECKPOINT_DIR
...
def load_model_deberta(adapter_dir=None, device=None):
    from peft import PeftModel
    from transformers import AutoModelForSequenceClassification, AutoTokenizer
    from train_deberta import BASE_MODEL, CHECKPOINT_DIR
    ...
    tok = AutoTokenizer.from_pretrained(BASE_MODEL)
    # attn_implementation="eager": serve per poter leggere le attention weights
    # (con SDPA/flash-attention output_attentions viene ignorato)
    base = AutoModelForSequenceClassification.from_pretrained(
        BASE_MODEL, num_labels=2, ignore_mismatched_sizes=True, attn_implementation="eager"
    )
    model = PeftModel.from_pretrained(base, adapter_dir).to(device)
    model.eval()
    return model, tok, device
...
def load_model_qwen(adapter_dir=None, device=None):
    from peft import PeftModel
    from transformers import AutoModelForSequenceClassification, AutoTokenizer, BitsAndBytesConfig
    from train_qwen import BASE_MODEL, CHECKPOINT_DIR
    ...
    base = AutoModelForSequenceClassification.from_pretrained(
        BASE_MODEL, num_labels=2, ignore_mismatched_sizes=True,
        quantization_config=bnb_config, device_map={"": 0}, attn_implementation="eager",
    )
    base.config.pad_token_id = tok.pad_token_id
    model = PeftModel.from_pretrained(base, adapter_dir)
    model.eval()
...
MODEL_LOADERS = {
    "FCNN":      load_model_fcnn,
    "PaperCNN":  load_model_papercnn,
    "daBERTa":   load_model_deberta,
    "Qwen":      load_model_qwen,
}